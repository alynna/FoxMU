...
if(!String.linkify) {
	...
	// Specify an inline image
	var imgPattern = /\b(img)(s?:\/\/[a-z0-9-+&@#\/%?=~_|!:,.;]*[a-z0-9-+&@#\/%=~_|])(.jpe?g|.png|.gif|.bmp)/gim;
	var imgpPattern = /\b(http)(s?:\/\/[a-z0-9-+&@#\/%?=~_|!:,.;]*[a-z0-9-+&@#\/%=~_|])(.jpe?g|.png|.gif|.bmp)/gim;
	var imgsPattern = /\b(imgp)(s?:\/\/[a-z0-9-+&@#\/%?=~_|!:,.;]*[a-z0-9-+&@#\/%=~_|])(.jpe?g|.png|.gif|.bmp)/gim;

        // Email addresses
        var emailAddressPattern = /\w+@[a-zA-Z_]+?(?:\.[a-zA-Z]{2,6})+/gim;

        return this
	    .replace(imgpPattern, 'imgp$2$3') // Temporarily convert image links to imgp:// URL
            .replace(urlPattern, '<a href="$&" style="color:#00ff00;underline;" target="_blank">$&</a>') // Make standard URL's clickable..
	    // convert img and imgs URLs to inline images
            .replace(imgPattern, '<a href="http$2$3" style="color:#00ff00;underline;" target="_blank"><img src="http$2$3" height="10%" /></a><div class="box"><img src="http$2$3" height="100%" frameborder="0" style="overflow:visible;border:0px;float:right;" /></div>')
	    // convert email addresses to mailto links
            .replace(emailAddressPattern, '<a href="mailto:$&" style="color:#00ff00; underline;" target="_blank">$&</a>')
	    // process imgp URL.
	    .replace(imgsPattern, '<a href="http$2$3"  style="color:#00ff00;underline;" target="_blank">http$2$3</a><div class="box"><img src="http$2$3" height="100%" frameborder="0" style="overflow:visible;border:0px;float:right;" /></div>');
...
/*
if(!String.mucode) {
    String.prototype.mucode = function() {

        // Find
        var urlPattern = /\b(?:imgs?):\/\/[a-z0-9-+&@#\/%?=~_|!:,.;]*[a-z0-9-+&@#\/%=~_|]/gim;
        var imgPat = /img:\/\/(.+?)\b/gim;
        var imgsPat = /imgs:\/\/=(.+?)\b/gim;

	// Replace
        return this
            .replace(imgPat, '<a href="http://$1"><img src="http://$1" height="10%" /></a><div class="box"><img src="http://$1" height="100%" frameborder="0" style="overflow:visible;border:0px;" /></div>')
            .replace(imgsPat, '<a href="https://$1"><img src="https://$1" height="10%" /></a><div class="box"><img src="https://$1" height="100%" frameborder="0" style="overflow:visible;border:0px;" /></div>');
    };
}
*/
...
var FoxMU_UI = {
    ...
    init: function () {
        ...
        this.icons = [["i-save", "log.png", function(){FoxMU_Menu.saveLog();}],
                      ["i-clear", "configure.png", function(){FoxMU_Menu.options();}],
                      ["i-conn-on", "Button-icon.gif", function(){FoxMU_Menu.connection();}],
                      ["i-conn-off", "Button-icon-off.gif", function(){FoxMU_Menu.connection();}]];
        this.initCmdLine();
        ...
        if (show_guide = readCookie("showGuide")) {
            if (show_guide == "yes")
                FoxMU_Menu.showGuide(true);
            else
                FoxMU_Menu.hideGuide(true);
        } else {
            FoxMU_Menu.showGuide(false);
        }
	...
	this.screenWrite("<br />Socket type: "+ FoxMU_Client.socktype+"<br />");
        FoxMU_Client.tryToConnect();
    },
    ...
    initCmdLine: function () {
        ...
        this.cmdline.onkeydown = function(e) {
            ...
            if (e.keyCode == 38) { // 38 = Up
                this.value = FoxMU_History.previous();
            }
            else if (e.keyCode == 40) { // 40 = Down
                this.value = FoxMU_History.next();
            } 
	    else if (e.keyCode == 33) {
		document.getElementById("output").scrollTop -= document.getElementById("output").clientHeight;
	    }
	...
	if (credentials) FoxMU_Client.command(credentials);
	FoxMU_UI.setEcho(readCookie("echo"));
    },
    ...
    command: function (cmd) {
        ...
        FoxMU_History.add(cmd);
        FoxMU_Client.command(cmd);
    },
	...
	dummy.innerHTML = ansi.ansi_to_html(str.linkify());
    ...
    rearrange: function() {
        ...
        /* output window */
        var o = this.output;
        o.style.height = (d.height - (c.offsetHeight + 1) - 90) + "px";
    ...
    setEcho: function (b) {
        ...
        this.echo = b;
	document.getElementById("CommandEcho").innerHTML = "Command Echo ["+(this.echo ? "ON":"OFF")+"]";
...
var FoxMU_Menu = {
    connection: function () {
        ...
        if (cm.style.display == "none") {
            ...
            if (FoxMU_UI.connState == "disconnected") {
                li.appendChild(d.createTextNode(m.connect));
                li.onclick = function(){FoxMU_Client.connect();};
                ul.appendChild(li);
            } else {
                li.appendChild(d.createTextNode(m.reconnect));
                li.onclick = function(){FoxMU_Client.disconnect();FoxMU_Client.connect();};
                ul.appendChild(li);
    
                var li2 = d.createElement("li");
                li2.appendChild(d.createTextNode(m.disconnect));
                li2.onclick = function(){FoxMU_Client.disconnect();};
                ul.appendChild(li2);
            }
    ...
    updateOptionsMenu: function () {
        ...
        g_li1.onclick = function(){FoxMU_Menu.showGuide(true);};
        g_li1.appendChild(d.createTextNode(m.cmdGuide));
        g_ul.appendChild(g_li1);
        var g_li2 = d.createElement("li");
        g_li2.onclick = function(){window.open(FoxMU_Config.MUHelpURL);};
        g_li2.appendChild(d.createTextNode("MUCK Help"));
        g_ul.appendChild(g_li2);
        var g_li3 = d.createElement("li");
        g_li3.onclick = function(){window.open("http://"+window.location.hostname+"/help/");};
        g_li3.appendChild(d.createTextNode("FoxMU Help"));
        g_ul.appendChild(g_li3);
        om.appendChild(g_fs);

        /* settings fieldset */
        var s_fs = d.createElement("fieldset");
        var s_lg = d.createElement("legend");
        s_lg.appendChild(d.createTextNode(m.settings));
        s_fs.appendChild(s_lg);
	var s_ul = d.createElement("ul");
	s_fs.appendChild(s_ul);

        var s_li1 = d.createElement("li");
        s_li1.onclick = function(){
	    var credentials=prompt("Please enter your connect line for autologin, or blank this field to clear it.\nUsually: connect <loginname> <password>",readCookie("credentials"));
	    if (credentials) {
		createCookie("credentials",credentials,365);
	    } else {
		eraseCookie("credentials");
	    }
	};
        s_li1.appendChild(d.createTextNode(m.autoLogin));
        s_ul.appendChild(s_li1);

        var s_li2 = d.createElement("li");
        s_li2.onclick = function(){
	    FoxMU_UI.setEcho(!(FoxMU_UI.echo));
	    if (FoxMU_UI.echo)
		createCookie("echo",FoxMU_UI.echo,365);
	    else
		eraseCookie("echo");
	};
	s_li2.id = "CommandEcho";
        s_li2.appendChild(d.createTextNode("Command Echo ["+(FoxMU_UI.echo ? "ON":"OFF")+"]"));
        s_ul.appendChild(s_li2);
        ...
        f_li1.onclick = function(){FoxMU_UI.selectFont('font-fixed', true);};
        f_li1.appendChild(d.createTextNode("Fixedsys"));
        f_ul.appendChild(f_li1);
        var f_li2 = d.createElement("li");
        f_li2.onclick = function(){FoxMU_UI.selectFont('font-courier', true);};
        f_li2.appendChild(d.createTextNode("Courier New"));
        f_ul.appendChild(f_li2);
        var f_li3 = d.createElement("li");
        f_li3.onclick = function(){FoxMU_UI.selectFont('font-courier-b', true);};
        f_li3.appendChild(d.createTextNode("Courier New Bold"));
        f_ul.appendChild(f_li3);
        var f_li4 = d.createElement("li");
        f_li4.onclick = function(){FoxMU_UI.selectFont('font-monospace', true);};
        f_li4.appendChild(d.createTextNode("Monospace"));
        f_ul.appendChild(f_li4);
        f_fs.appendChild(f_ul);
        var f_li5 = d.createElement("li");
        f_li5.onclick = function(){FoxMU_UI.selectFont('font-console', true);};
        f_li5.appendChild(d.createTextNode("Console"));
        f_ul.appendChild(f_li5);
        f_fs.appendChild(f_ul);
	/*
        var f_li6 = d.createElement("li");
        f_li6.onclick = function(){FoxMU_UI.selectFont('font-sansmono', true);};
        f_li6.appendChild(d.createTextNode("Sans Mono"));
        f_ul.appendChild(f_li5);
        f_fs.appendChild(f_ul);
	*/
    ...
    saveLog: function () {
        ...
        if (ls.style.display == "none") {
            ...
            lt.value = "<html><head><title>Raw Log</title><style type=\"text/css\">"
                + document.getElementById("logStyle").innerHTML
                + "</style><body" + bodyClass + "><pre id=\"output\">"
                + FoxMU_UI.output.innerHTML + "</pre></body></html>";
    ...
    showGuide: function (setCookie) {
        ...
        for (var i = 0; i < FoxMU_Guide.length; ++i) {
            ...
            /* section */
            th.appendChild(d.createTextNode(FoxMU_Guide[i][0]));

            /* commands in this section */
            var commands = FoxMU_Guide[i][1];
            ...
            for (var j = 0; j < commands.length; ++j) {
                ...
                sp1.onmouseover = function() {
                    ...
                    if ((findPos(e)[1] + e.offsetHeight) > (findPos(FoxMU_UI.output)[1] + FoxMU_UI.output.offsetHeight))
                ...
                sp1.onclick = function() { FoxMU_UI.cmdline.value += this.firstChild.nodeValue + " "; };
                td.appendChild(sp1);
